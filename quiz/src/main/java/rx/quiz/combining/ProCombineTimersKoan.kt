 ...
 * * Intermediate knowledge about operator composition
 */
abstract class ProCombineTimersKoan {
    /**
     * @see [ProCombineTimersKoan]
     */
    ...
    @Test
    fun `Observable must be subscribed only once`() {
        val scheduler = TestScheduler()
        var refCount = 0
        Observable
                .defer {
                ...
                .test().apply {
                    scheduler.advanceTimeBy(100, TimeUnit.SECONDS)
                    assertThat(refCount)
                            .describedAs("Amount of subscribers")
                            .isEqualTo(1)
                }
    }
}